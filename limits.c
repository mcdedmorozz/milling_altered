//  limits.c - Код, относящийся к предельным переключателям и выполняющий цикл самонаведения
...
void limits_init() 
{
  LIMIT_DDR &= ~(LIMIT_MASK); // Установить как входные контакты

  #ifdef DISABLE_LIMIT_PIN_PULL_UP
    LIMIT_PORT &= ~(LIMIT_MASK); //Нормальный низкий режим. Требуется внешнее вытягивание. Normal low operation. Requires external pull-down.
  #else
    LIMIT_PORT |= (LIMIT_MASK);  // Включите внутренние подтягивающие резисторы. Нормальная высокая работа. Enable internal pull-up resistors. Normal high operation.
  #endif
...
// your e-stop switch to the Arduino reset pin, since it is the most correct way to do this.
  ...
  ISR(LIMIT_INT_vect) // DEFAULT: Прерывание процесса смены предельных значений.
  {
    // Игнорируйте конечные выключатели, если они уже находятся в состоянии тревоги или в процессе выполнения тревоги.
    // Когда в состоянии тревоги Grbl должен быть сброшен или будет принудительно перезагружен, поэтому любые ожидающие 
    // перемещения в планировщике и последовательных буферах будут очищены, и вновь отправленные блоки будут
    // заблокированы до цикла самонаведения или блокировки блокировки. Позволяет пользователю отключить жесткий
    // предела, если их пределы постоянно запускаются после сброса и перемещают их оси.
    if (sys.state != STATE_ALARM) { 
      if (!(sys_rt_exec_alarm)) {
        #ifdef HARD_LIMIT_FORCE_STATE_CHECK
          // Проверить состояние пинов жёстких границ. 
          if (limits_get_state()) {
            mc_reset(); // Initiate system kill.
            bit_true_atomic(sys_rt_exec_alarm, (EXEC_ALARM_HARD_LIMIT|EXEC_CRITICAL_EVENT)); // Указать критическое событие с жестким пределом
          }
        #else
          mc_reset(); // Initiate system kill.
  ...
  // При изменении пина жёсткого предела, включите сторожевой таймер, чтобы создать короткую задержку. 
  ISR(LIMIT_INT_vect) { if (!(WDTCSR & (1<<WDIE))) { WDTCSR |= (1<<WDIE); } }
  ISR(WDT_vect) // Сторожевой таймер ISR
  {
    WDTCSR &= ~(1<<WDIE); //Отключить сторожевой таймер. 
...
{
  if (sys.abort) { return; } // Заблокируйте, если был произведен сброс системы. Block if system reset has been issued.
  ...
  uint8_t idx;
  for (idx=0; idx<N_AXIS; idx++) {  
    // Инициализация шаговых контактных масок Initialize step pin masks
