...
static char line[LINE_BUFFER_SIZE]; // Строка, которая должна быть выполнена. Нулевое завершение.
...
/*
Направляет и выполняет одну строку форматированного ввода из протокола protocol_process. Хотя в основном 
поступающие потоковые блоки g-кода, это также направляет и выполняет внутренние команды Grbl,
такие как настройки, инициирование цикла самонаведения и переключения состояний коммутатора.
*/
static void protocol_execute_line(char *line) 
{      
  protocol_execute_realtime(); //Контрольная точка команды выполнения.
  if (sys.abort) { return; } // Вызов функции вызова при прерывании системы
  ...
  if (line[0] == 0) {
    // Пусто или комментарий. Отправить сообщение о состоянии для целей синхронизации.
    ...
    // Все остальное - gcode. Блокировать, если в режиме тревоги.
    report_status_message(STATUS_ALARM_LOCK);

  } else {
    // Разберите и выполните блок g-кода!
...
*/
// Выполняет команды времени выполнения, если требуется. Это вызывается из различных контрольных точек в основной
// программе, прежде всего там, где может быть цикл while, ожидающий освобождения буфера или любой
// точки, где время выполнения от последней контрольной точки может быть больше чем доли секунды.
// Это способ асинхронного выполнения команд реального времени (так называемая многозадачность) с помощью функций анализа и планирования g-кода grbl.
// Эта функция также служит интерфейсом для прерываний для установки флагов реального времени системы,
// в которых обрабатывается только основная программа, избавляя от необходимости
// определять более дорогостоящие переменные для вычислений. Это также обеспечивает контролируемый способ
// выполнения определенных задач, не имея двух или более экземпляров одной и той же задачи, например планировщика,
// пересчитывающего буфер по каналу или переопределения.
// ПРИМЕЧАНИЕ: Флаги переменных sys_rt_exec_state устанавливаются любыми прерываниями процесса, 
// шага или последовательного порта, распиновками, конечными ключами или основной программой.
void protocol_execute_realtime()
{
  uint8_t rt_exec; // Temp variable to avoid calling volatile multiple times.

  do { //Если система приостановлена, здесь выполняется перезапуск цикла приостановки.
    
	// Проверка и выполнение аварийных сигналов.
...
//         }
  ...
  //Перезагрузить буфер сегментного сегмента
