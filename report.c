/*
  report.c - методы отчетов и сообщений
...
*/
/*
Этот файл работает как первичный интерфейс обратной связи для Grbl. Здесь хранятся любые исходящие данные, такие 
как сообщения о состоянии протокола, сообщения обратной связи и отчеты о состоянии.
По большей части эти функции в основном вызываются методами protocol.c. Если
требуется другая обратная связь по стилю (например, JSON), то пользователь может изменить следующие
способы для удовлетворения своих потребностей.
*/
#include "grbl.h"
...
*/
// Печатает текущие параметры датчика координат. По команде зонда эти параметры обновляются после 
// успешного определения координат или после неудачного зонда с командой G38.3 без ошибок (если поддерживается). 
// Эти значения сохраняются до тех пор, пока Grbl не отключится, в результате чего они будут обнулены.
...
void report_probe_parameters()
{
  ...
  // Отчет в терминах положения машины.
