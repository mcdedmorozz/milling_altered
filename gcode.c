...
// ПРИМЕЧАНИЕ. Максимальный номер строки определяется стандартом g-кода как 99999. Похоже, это 
// произвольное значение, и для некоторых графических интерфейсов может потребоваться больше. Поэтому мы увеличили его, 
// основываясь на максимальном безопасном значении, при преобразовании числа с плавающей запятой (точность 7,2 цифр) в целое.
#define MAX_LINE_NUMBER 9999999 

#define AXIS_COMMAND_NONE 0
#define AXIS_COMMAND_NON_MODAL 1 
#define AXIS_COMMAND_MOTION_MODE 2
#define AXIS_COMMAND_TOOL_LENGTH_OFFSET 3 // *Undefined but required (Не определено, но требуется)

// Declare gc extern struct (Объявление структур)
...
void gc_init() 
{
  ...
  // Загружаем систему координат G54 по умолчанию.
...
// Устанавливает позицию парсера g-кода в мм. Ввод пошагово. Вызывается прерываниями системы и жесткими ограничениями.
...
uint8_t gc_execute_line(char *line) 
{
  /* -------------------------------------------------------------------------------------
	...
	/* -------------------------------------------------------------------------------------
	ШАГ 1: инициализировать структуру блока парсера и скопировать текущие режимы состояния g-кода. Парсер 
	обновляет эти режимы и команды, так как блок-строка является синтаксическим анализатором, и будет использоваться и 
	выполняться только после успешной проверки ошибок. Структура блока парсера также содержит структуру 
	значений блоков, переменные отслеживания слов и немодальный командный трекер для нового 
	блока. Эта структура содержит всю необходимую информацию для выполнения блока. */

  memset(&gc_block, 0, sizeof(parser_block_t)); // Инициализировать структуру блока парсера.
  memcpy(&gc_block.modal,&gc_state.modal,sizeof(gc_modal_t)); // Копирование текущих режимов
  ...
  while (line[char_counter] != 0) { // Loop until no more g-code words in line.
    ...
    switch(letter) {
      ...
      case 'G':
        // Определить команду «G» и ее модальную группу
        switch(int_value) {
  ...
  */
/*
Ошибка - проверить все команды и значения, переданные в этом блоке. Этот шаг гарантирует, что все
 команды действительны для выполнения и максимально соответствуют стандарту NIST.
Если обнаружена ошибка, все команды и значения в этом блоке сбрасываются и не будут обновлять режимы 
активного кода системы. Если блок в порядке, активные режимы g-кода будут обновлены на основе
 команд этого блока и будут сигналом для его выполнения.

Кроме того, мы должны предварительно преобразовать все значения, переданные на основе режимов, 
установленных анализируемым блоком. Существует ряд проверок ошибок, для которых требуется целевая информация, 
которую можно точно рассчитать, только если мы преобразуем эти значения вместе с проверкой ошибок.
Это переводит следующий этап выполнения, как только обновление режимов g-кода системы и выполнение 
запрограммированных действий по порядку. На этапе выполнения не требуются какие-либо расчеты преобразования
 и требуется только минимальная проверка, необходимая для выполнения.
*/
  ...
  */  
  ...
  // Определить условия неявной команды оси. Оси слова были переданы, но явная команда оси не была отправлена.
  // Если да, установите команду оси в текущий режим движения.
  if (axis_words) {
  ...
  // [15. Выбор системы координат ]: *N/A.   Ошибка, если активен радиус резца.
  // TODO: Чтение EEPROM данных координат может потребовать синхронизации буфера, когда цикл активен.
  // Чтение приостанавливает работу процессора и может вызвать редкий сбой.
  // Для будущих версий на процессорах с достаточным объемом памяти все координатные данные должны 
  // храниться в памяти и записываться в EEPROM только в том случае, если нет активного цикла.
  ...
  // [16. Задать режим управления траекторией ]: N/A. Only G61. G61.1 and G64 NOT SUPPORTED.
  // [17. Задать режим расстояния ]: N/A. Only G91.1. G90.1 NOT SUPPORTED.
  // [18. Режим вставки ]: НЕ ПОДДЕРЖИВАЕТСЯ.
  
  // [19. Оставшиеся немодальные действия ]: Проверьте переход в предопределенное положение, установите G10 или установите смещение оси.
